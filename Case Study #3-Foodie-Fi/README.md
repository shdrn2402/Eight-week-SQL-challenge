...
- Subquery (`customer_id NOT IN (...)`):
  - Identifies all `customer_id` values associated with churn (`plan_id = 4`).
...
SELECT
  plan_name,
  SUM(amount) AS total_revenew_usd
FROM
  payments
GROUP BY
  plan_name
ORDER BY
  total_revenew_usd DESC;
```

<details>
  <summary><em>show description</em></summary>

This query calculates the total revenue (`total_revenew_usd`) generated by each subscription plan (`plan_name`) and sorts the results in descending order of revenue.

- `SELECT` clause:
  - `plan_name`: Represents the name of the subscription plan.
  - `SUM(amount) AS total_revenew_usd`: Calculates the total payment amount for each subscription plan, indicating the total revenue.
...
- `ORDER BY total_revenew_usd DESC`:
...
- `total_revenew_usd`: Total revenue generated by that plan.
